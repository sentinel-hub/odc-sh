...
class SearchResponse:
    ...
    def print_info(self, **kwargs):
        ...
        if aoi and len(self.data.features) > 0:
            ...
            aoi_polygon = (
                Polygon(aoi["coordinates"][0]) if type(aoi) is dict else Polygon(aoi)
            )
            aoi_area = aoi_polygon.area

            intersects = [geom.intersection(aoi_polygon) for geom in data.geometry]
            intersects_perc = [
                intersect.area / aoi_area * 100 for intersect in intersects
            ]
        ...
        if self.typ == SkySatType or self.typ == ScopeType:
            print_data = [
                self.print_fun(
                    feature["properties"],
                    [idx, feature["id"]],
                    intersects_perc[idx] if len(intersects_perc) > 0 else -1,
                )
...
class BaseCommercialClient(SentinelHubService):
    ...
    def _call_job(self, endpoint_name: str, payload: dict) -> Json:
        ...
        return self.client.get_json_dict(
            url=job_url,
            post_values=payload,
    ...
    def search(self, payload) -> CommercialSearchResponse:
        response_info: CommercialSearchResponse = CommercialSearchResponse.from_dict(
            self._call_job("search", payload)
        )
        return response_info

    def native_search(self, payload) -> CommercialSearchResponse:
        response_info: CommercialSearchResponse = CommercialSearchResponse.from_dict(
            self._call_job("nativesearch", payload)
        )
        return response_info
...
class SentinelHubCommercialData(BaseCommercialClient):
    ...
    def search_airbus(
        ...
        **kwargs: Any,
        ...
        payload = dict(
            provider=Providers.AIRBUS.value,
            bounds=self.get_bounds(bounds),
            data=[{"constellation": typ.value}],
        )
        ...
        return SearchResponse(
            self.search(payload),
            query=payload,
            thumb=self.get_thumb_type(typ.value),
            typ=type(typ),
    ...
    def search_worldview(
        ...
        **kwargs: Any,
        ...
        kwargs = dict(kwargs, timeRange={"from": time_from, "to": time_to})
        payload = dict(
            provider=Providers.WORLDVIEW.value,
            bounds=self.get_bounds(bounds),
            data=[{"productBands": "4BB", "productKernel": kernel.value}],
        )
        payload["data"][0]["dataFilter"] = dict()
        for name, val in kwargs.items():
            payload["data"][0]["dataFilter"][name] = val
        ...
        return SearchResponse(
            self.search(payload),
            query=payload,
            thumb=self.get_thumb_type("WORLDVIEW"),
            typ=type(kernel),
            search_props=search_props,
        )
    ...
    def search_planet(
        self,
        typ: Union[SkySatType, ScopeType],
        bundle: Union[ScopeBundle, SkySatBundle],
        bounds: dict,
        time_from: str,
        time_to: str,
        **kwargs: Any,
        ...
        kwargs = dict(kwargs, timeRange={"from": time_from, "to": time_to})
        payload = dict(
            provider=Providers.PLANET.value,
            bounds=self.get_bounds(bounds),
    ...
    def thumbnail(self, provider_type: ThumbnailType, item_id):
        download_request = DownloadRequest(
            ...
            + f"/collections/{provider_type}/products/{item_id}/thumbnail",
            request_type=RequestType.GET,
            use_session=True,
        )
        return self.client.download(
            download_requests=download_request, decode_data=False
        )
    ...
    def order(self, name: str, query: dict, **kwargs):
        ...
        try:
            return SearchResponse(
                self._call_job("orders", payload),
                typ="Single",
                search_props=search_props,
            )
        except:
            return SearchResponse(
                "", err="Error creating order. Check if collection name already exists."
            )

    def get_orders(self, *args):
        response_info = self.client.get_json_dict(
            url=self.service_url + "/orders" + self.check_arg(args),
            request_type=RequestType.GET,
            use_session=True,
        )

        response_info = (
            dict(data=[response_info])
            if not response_info.get("data")
            else response_info
        )
        search_props = ["id", "created", "name", "provider", "sqkm", "status"]
        return SearchResponse(
            response_info.get("data"), typ="Normal", search_props=search_props
        )

    def delete_order(self, id_order):
        try:
            self.client.get_json(
                url=self.service_url + "/orders/" + id_order,
    ...
    def confirm_order(self, id_order):
        try:
            data = self.client.get_json(
                url=self.service_url + "/orders/" + id_order + "/confirm",
                request_type=RequestType.POST,
                use_session=True,
            )
            search_props = ["id", "created", "name", "provider", "sqkm", "status"]
