...
from CommercialDataBase import CommercialSearchResponse, T, BaseAirbusResponse, ThumbnailType, \
    AirbusConstellation, Providers, ScopeType, ScopeBundle, SkySatBundle, SkySatType, OrderType
...
class BaseCommercialClient(SentinelHubService):
    ...
    def get_bounds(self, bounds: Any):
        btype = "geometry" if bounds["type"] else "bbox"
...
class SentinelHubCommercialData(BaseCommercialClient):
    ...
    def search_airbus(self, typ: AirbusConstellation, bounds: dict, time_from: str, time_to: str, **kwargs: Any):
        ...
        payload = dict(type=Providers.AIRBUS.value, bounds=self.get_bounds(bounds), data=[{"constellation": typ.value}])
    ...
    def search_planet(self, typ: Union[SkySatType, ScopeType], bundle: Union[ScopeBundle, SkySatBundle], bounds: dict, time_from: str,
        ...
        planet_api_key = kwargs.get("planetApiKey") if kwargs.get("planetApiKey") else ""
        kwargs.pop("planetApiKey")
        kwargs = dict(kwargs, timeRange={"from": time_from, "to": time_to})
        payload = dict(type=Providers.PLANET.value, bounds=self.get_bounds(bounds),
                       data=[{"itemType": typ.value, "productBundle": bundle.value}], planetApiKey=planet_api_key)
    ...
    def order(self, name: str, collection_id: str, query: dict, **kwargs):
        ...
        if provider == Providers.PLANET:
            query['data'][0]["harmonizeTo"] = "NONE"

        item_ids = kwargs.get("item_ids")
        if item_ids:
            del query["data"]["dataFilter"]
            if provider == Providers.AIRBUS:
                for itm_id in kwargs.get("item_ids"):
                    query["data"]["products"].append({"id":itm_id})
            elif provider == Providers.PLANET:
                query["data"]["itemIds"] = item_ids
